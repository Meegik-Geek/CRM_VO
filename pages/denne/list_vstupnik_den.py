...
class ListVstupnikDen(QWidget):
    ...
    def populate_specialty_combo(self):
        """Завантаження спеціальностей через репозиторій."""
        repo = SpecialtyRepository()
        try:
            specialties = repo.get_specialties(form_type='day')
            self.specialty_combo.clear()
            for spec in specialties:
                # Використовуємо назву прямо з бази, бо вона вже містить код
                display_text = spec[1]
                self.specialty_combo.addItem(display_text, spec[1]) 
        except Exception as e:
            log_error("Помилка завантаження спеціальностей (денне)", e)
            show_error(self, "Не вдалося завантажити список спеціальностей.")
    ...
    def search_vstupnik(self):
        ...
        if not selected_specialty:
            # Якщо раптом дані відсутні, спробуємо розпарсити текст (Fallback)
        ...
        try:
            results = repo.search_applicants_by_specialty(selected_specialty, form_type='day')
            if results:
